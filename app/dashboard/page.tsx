...
function useTabParam(defaultTab: string) {
  const router = useRouter()
  const params = useSearchParams()
  const raw = params.get("tab")
  const current = raw === null || raw === "" ? "registrar" : raw
  const setTab = React.useCallback(
    (tab: string) => {
      ...
      const qs = qp.toString().replace(/tab=$/, "tab")
...
export default function DashboardPage() {
  const { current, setTab } = useTabParam("registrar")
      ...
      <nav aria-label="Dashboard sections" className="mb-6 overflow-x-auto">
        <ul className="flex items-center gap-1">
          {[
            ...
            { key: "registrar", label: "SPMB" },
          ].map((item) => {
            const href = item.key === "registrar" ? "/dashboard?tab" : `/dashboard?tab=${item.key}`
          ...
          <Card>
            ...
            <CardContent className="grid gap-4">
              <Tabs defaultValue="schema" className="w-full">
                <TabsList>
                  <TabsTrigger value="schema">Kelola Formulir Online</TabsTrigger>
                  <TabsTrigger value="applicants">Data Pendaftar</TabsTrigger>
                </TabsList>
